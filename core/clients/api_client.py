...
class APIClient:
    ...
    def ping(self):
        ...
        with allure.step('Assert status code'):
            assert response.status_code == 201, f"Excepted status 201 but got {response.status_code}"
    ...
    def auth(self):
        ...
        with allure.step('Checking status_code'):
            assert response.status_code == 200, f"Excepted status 200 but got {response.status_code}"
    ...
    def get_booking_by_id(self):
        with allure.step('Get booking by ID'):
            url = f"{self.base_url}{Endpoints.BOOKING_ENDPOINT}{TestBookingData.BOOKING_ID}"
            response = self.session.get(url)
            response.raise_for_status()
        with allure.step('Checking status_code'):
            assert response.status_code == 200, f"Expected status 200 but got {response.status_code}"
        with allure.step('Verify booking data'):
            assert response == TestBookingData.EXPECTED_BOOKING_DATA